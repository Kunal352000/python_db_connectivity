...
def updateData(n):
    ...
    s1=StringVar()
    u1=Label(f7,font=("",11),text="Enter R.NO",bg="#091e42",fg="white")
    u1.place(x=100,y=50)
    ...
    def updatedata1():
        ...
        for r1 in r:
            ...
            u5=Label(f7,font=("",11),text="python is",bg="#091e42",fg="white")
            u5.place(x=200,y=150)
            u6=Entry(f7,font=("",11),textvariable=s3,bg="#091e42",fg="white")
            u6.insert(0,r1[2])
            u6.place(x=300,y=150)

            u7=Label(f7,font=("",11),text="DS&A is",bg="#091e42",fg="white")
            u7.place(x=200,y=200)
            u8=Entry(f7,font=("",11),textvariable=s4,bg="#091e42",fg="white")
            u8.insert(0,r1[3])
            u8.place(x=300,y=200)

            u9=Label(f7,font=("",11),text="c++ is",bg="#091e42",fg="white")
            u9.place(x=200,y=250)
            ...
            def updatedata2():
                db=sqlite3.connect('python.db')
                cr=db.cursor()
                cr.execute("update ins set UNAME = '"+s2.get()+"',UPHY = '"+s3.get()+"',UDS = '"+s4.get()+"',UC = '"+s5.get()+"' where URNO='"+s1.get()+"'" )
                db.commit()
                db.close()
                showalldata1(f55)
...
def deleteData(n):
    ...
    u1=Label(f8,font=("",11),text="Enter R.NO",bg="#091e42",fg="white")
    u1.place(x=100,y=50)
